    ...
    def __init__(
        ...
        for i in range(depth):
            ...
            if i in last_block_indexes:
                name = f"res{stage}"
                self.num_features.append(dim_out)
                self.stride_features.append(stride)
...
class D2MViT(MViT, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        super().__init__(
            img_size=img_size,
            embed_dim=embed_dims[0],  # default is list, here uses int
            depth=depths[0],  # default is list, here uses int
            num_heads=num_heads[0],  # default is list, here uses int
            last_block_indexes=out_indices,
            residual_pooling=True,
        ...
        self._out_feature_strides = {
            "res2": self.stride_features[0],
            "res3": self.stride_features[1],
            "res4": self.stride_features[2],
            "res5": self.stride_features[3],
        }
